...
from leads_data import LEADS

SRC = "/tmp/gen/master-template-v2.html"
with open(SRC) as f:
    MASTER = f.read()
...
# ---------- before/after: user-supplied AI-generated placeholder photos ----------
# Placeholder pair, explicitly flagged by the user for later replacement.
# Per user's confirmed final assignment (swapped from the initial guess):
#   BEFORE slot = the brighter/tidier-looking generated image (after.jpg)
#   AFTER slot  = the darker/messier-looking generated image (before.jpg)
...
# ---------- gallery fallback illustrations (last-resort only, if a business has 0 real photos) ----------
...
# ---------- server-baked testimonial marquee ----------
